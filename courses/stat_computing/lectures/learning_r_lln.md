...
Specifically, LLN says that sample averages based on a larger
sample size will be closer to the population average than sample averages 
base on a smaller sample size.
...
#### What is a function?
A function in programming is similar to a function in mathematics $$f(x)=y$$.
...
- The type of the elements within a vector is the vector's **type** (implication of
  this statement is that elements within a vector must all share the same type).
  - `class(dice)`
