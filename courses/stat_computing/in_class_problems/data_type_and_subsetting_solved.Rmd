...
head(block_data, 3)
```
- We see the output states 13 obs and 9 variables so we have 13 row and 9 columns in `block_data`.
- Each column is of type `int`
- Randomly choosing a few entries and checking against Table 3 checks out.
...
We will walk you through each step below to highlight 
...
f_timing[has_no_fert] <- missing_timing
f_timing
```
...
- Were the data types of your inputs the same across the 2 methods?
  - method 1 used both a character vector and a numeric vector, method 2 inputs were a character then boolean vector.
