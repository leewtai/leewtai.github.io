...
def parse_name(family_name, given_name='', middle_name=''):
    ...
    if not middle_name and re.findall('\\.', given_name):
        split_name = [letter for letter in given_name.split('.')
                      if letter.isalpha()]
        if len(split_name) == 2:
            given_name, middle_name = split_name
...
def merge_author(graph, family_name, given_name='',
    ...
    if not results:
        ...
        logging.warning('Updating {} {} {} to {} {} {}'.format(
            record_name['given_name'], record_name['middle_name'],
            record_name['family_name'], given_name, middle_name, family_name))
        ...
        [logging.warning('{} {} {}'.format(a['given_name'], a['middle_name'],
                                           a['family_name'])) for a in results]
...
def merge_referenced(graph, title, ref_title, year=0, ref_count=0):
    title = cln_title(title)
    ...
    if ref_count:
        graph.run("""MATCH (:Paper {{title: "{title}"}})"""
                  """-[r:REFERENCED]->"""
                  """(:Paper {{title: "{ref_title}"}})"""
                  "SET r.ref_count={ref_count}".format(
                      title=title, ref_title=ref_title,
                      ref_count=ref_count))
